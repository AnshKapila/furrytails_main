...
import { getShopContent } from '@/services/api';
...
// Headings and filter options are editorial content from src/data/home.ts, so
// this stays synchronous. Products arrive as a prop, fetched server-side.
...
function ShopContent({ products }: { products: WooProduct[] }) {
  ...
  //
  // Deliberately NOT initialised from useSearchParams(): reading search params
  // during render forces this entire subtree to be client-rendered, which took
  // the product grid out of the server HTML (bad for /shop indexing). The
  // ?ritual= deep link is applied just after mount instead — see below.
  ...
  // Apply the ?ritual= deep link once mounted, and keep it in sync with
  // back/forward navigation. Reading location directly keeps this out of the
  // render path so the grid can still be server-rendered.
  useEffect(() => {
    function syncFromUrl() {
      const r = new URLSearchParams(window.location.search).get('ritual');
